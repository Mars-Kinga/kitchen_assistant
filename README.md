...
AI 首次生成的候选及其通过本地校验的完整菜谱会自动保存到 `skills/kitchen_assistant/recipes/generated/`。同一个菜名的最多三个候选会合并在一个可读文件（例如 `cached_宫保鸡丁.json`）中；详情请求并行生成，后续在人数、口味与忌口条件相同且菜名匹配时，会优先读取本地缓存，`provider_mode` 为 `local_cache`，不会再次调用模型生成菜谱。缓存写入失败不会影响当前烹饪流程。程序启动时会清理旧版哈希命名的单菜谱缓存，避免旧文件与新菜谱重复。
...
步骤计时保留给依赖火候或等待时长的操作，例如煮、炖、焖、煎、烤、蒸、炸、焯、收汁，以及腌制、浸泡、泡发和静置。淘米、洗菜、切菜、打蛋、搅拌、调味和装盘即使模型返回了时长，本地标准化层也会清除该计时。用户可以说“开始了”“下锅了”或“给我计时”等自然表达启动当前步骤计时。

牛排会在收集人数后额外询问三分、五分、七分或全熟。AI 菜谱要求把正反面煎制拆成带 `duration_seconds` 的步骤：用户说“开始煎”后才启动正面计时，时间到时提醒翻面；用户确认“翻面好了”后启动另一面计时，结束后提示静置。普通肉类步骤若模型漏填时间，标准化器会补上保守的 2 分钟下限；时间到后只提醒检查熟度，不会自动推进步骤。文字交互模式会主动输出到时提醒。
...
标准化层还会把过密的备菜说明拆成容易记忆的小步骤，例如“腌肉”“泡发木耳”“切配蔬菜和姜蒜”分别指导。腌制、浸泡等等待步骤会提示用户利用计时准备下一步的调料或蔬菜；用户说“准备好了”只会记录辅助准备，不会提前推进腌制步骤。对于明显不合理的 AI 炒制时长，本地规则会设置保守下限：肉丝至少 120 秒并要求观察完全变色、中心无粉红；蔬菜炒至变软至少 180 秒，并明确以实际断生和软度为准，而不是把计时当成熟度保证。
